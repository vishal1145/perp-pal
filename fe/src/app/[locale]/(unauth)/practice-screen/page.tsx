...
import SubmitPopup, {
  SubmitPopupProps,
} from "@/components/PopupModal/SubmitPopup";
...
const PracticeScreen = () => {
  ...
  const getPracticePaper = async (id: string) => {
    try {
      const response = await axios.get(
        `${process.env.NEXT_PUBLIC_API_URI}/startassesment/${id}`
      );
      setQuestions(response?.data?.questions);
      setLoading(false);
      const initialPracticePaper = response?.data?.questions.map(
        (q: McqTestQuestion) => ({
          McqQuestion: q,
          userSelectAns: "",
  ...
  const submitPaper = async () => {
    ...
    try {
      ...
      const { data } = await axios.post(
        `${process.env.NEXT_PUBLIC_API_URI}/assessments`,
        {
          userId: userId,
  ...
  const showModal = (id: string): void => {
    ...
    for (let i = 0; i < userPracticePaper.length; i++) {
      if (userPracticePaper[i]?.userSelectAns != "") {
        totalAttempt++;
        if (
          userPracticePaper[i]?.McqQuestion?.correctAnswer ==
          userPracticePaper[i]?.userSelectAns
        ) {
          correct++;
        }
      }
  ...
  return (
    <>
      {resultScreen === true ? (
        <ResultPage userPracticePaper={userPracticePaper} />
      ) : (
        <>
            ...
            <div className="container mx-auto px-3">
              <div className="py-4">
                <div className="grid lg:grid-cols-[70%_30%]">
                  {isModalOpen && (
                    <SubmitPopup
                      title={submitPopupValue.title}
                      subTitle={submitPopupValue.subTitle}
                      total={submitPopupValue.total}
                      atemmpt={submitPopupValue.atemmpt}
                      correct={submitPopupValue.correct}
                      incorrect={submitPopupValue.incorrect}
                      message={submitPopupValue.message}
                      setIsModalOpen={submitPopupValue.setIsModalOpen}
                      submitAssessmentId={submitPopupValue.submitAssessmentId}
                  ...
                  <div className="flex flex-col">
                    ...
                    <div
                      className={`flex ${loading === true || index === 0 || submitLoading === true ? `justify-end` : `justify-between`}`}
                    >
                      <button
                        type="button"
                        className={`flex items-center ${index === 0 || submitLoading === true ? "hidden" : ""} hover:text-gray-700`}
                      ...
                      {loading ? (
                        ...
                        <button
                          type="button"
                          className={`text-white ${logoBtnColor} font-medium px-3 py-1.5 rounded`}
                        ...
                        <button
                          type="button"
                          className={`text-white ${logoBtnColor} rounded px-3 py-1.5 font-medium`}
                          onClick={nextQuestion}
                          disabled={loaderShow}
                        >
                          Submit
                    ...
                    {loading ? (
                      ...
                      <div className="my-4 rounded-lg shadow ">
                        <button
                          ...
                          className="inline-flex w-full items-center justify-between rounded bg-gray-100 px-4 py-2 text-sm font-normal transition-all duration-300 ease-in-out"
                        >
                        ...
                        <div
                          ref={panelRef}
                          className="overflow-hidden rounded bg-gray-100 font-normal text-gray-500 transition-all duration-700 ease-in-out"
                  ...
                  <div className="lg:ml-8">
                    ...
                    <div className="py-4" style={{ display: "none" }}>
                      <h3 className="text-sm font-medium ">Prepal Chat Bot</h3>
                      <p className="py-2 text-sm font-normal text-gray-500">
                        Integration / discuss
                      </p>
                    </div>
                  </div>

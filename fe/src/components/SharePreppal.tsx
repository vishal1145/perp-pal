...
const SharePreppal: React.FC<SharePreppalProps> = ({ setSharePreppal }) => {
    ...
    window.FB.ui(
      ...
      (response) => {
        if (response && !response.error_message) {
          ...
          setShareCount((prevCount) => prevCount + 1); // Increment only if shared
        } else {
          console.log("User canceled or there was an error.");
        }
      }
    );
  };
  ...
  const shareOnWhatsapp = () => {
    ...
    // Open WhatsApp and track if the user clicked the button, not if they actually shared
    const openedWindow = window.open(whatsappUrl, "_blank");

    if (openedWindow) {
      // Track if the window is opened successfully
      openedWindow.addEventListener("blur", () => {
        // Check if the user actually shared on WhatsApp
        // You can't directly track this, so assume success after opening
      ...
      <div className="relative px-4 pt-4 w-full max-w-3xl max-h-full">
        ...
        <div className="relative bg-white rounded-lg shadow">
          ...
          <div className="p-[20px] text-center">
            <p className={`${text1} `}>Did we make paper better for you?</p>
          ...
          <div className="flex flex-col sm:flex-row justify-center items-center gap-1 sm:gap-16 pb-[24px] pt-[4px]">
            <div className="flex flex-col items-center mb-6 sm:mb-0">
              <Image src={fb} className="h-10 w-10 mb-4" alt="Facebook Icon" />
              <button
            ...
            <div className="flex flex-col items-center">
              <Image src={whats} className="h-10 w-10 mb-4" alt="WhatsApp Icon" />
              <button
                className="mb-2 cursor-pointer rounded-lg border border-gray-500 bg-transparent px-5 py-2 text-sm font-medium hover:bg-gray-200 focus:outline-none"
                style={{
                  border: "1px solid rgb(226, 226, 226)",
          ...
          <div className="flex flex-col md:flex-row items-center justify-between p-4 md:p-5 border-t border-gray-200 rounded-b ">
            <div className="flex flex-col sm:flex-row gap-[14px] sm:gap-1">
              ...
              <button className="text-sm text-cyan-600 hover:text-cyan-700">Contact Support</button>
            </div>
